...
for key, value in dictionary2.iteritems():
    print "\t%s shares @ %s price\t $%r \t($%r)" % (
        shares,
        key,
        round(((value * btcprice) * shares), 2),
        value,
    )
...
for key, value in walletdictionary.iteritems():
    print "\t%s's Balance is\t B%s \t value: $%r Approx**" % (
        key,
        value,
        round(y * float(value), 2),
    )
...
target.truncate()
target.write("{0}".format(dictionary2))
target.close()
...
print "\tWeekly share rtn: %s = return @ %s%% of mining power" % (
    d,
    round(hashpercentage, 2),
)
print "\tWeekly share rtn: %s = return @ 100%% of mining power" % (
    round((float(d) / hashpercentage), 5)
)
tt = float(d) / hashpercentage
fullshareyield = shares * tt
fullsharereturn = fullshareyield * y
print "\tFull hash @ %s shares would = B%s = $%s @ %s price" % (
    shares,
    round(fullshareyield, 5),
    round(fullsharereturn, 3),
    a,
)
...
print "\nAnd finally"
print "\tIf you own %s of peta\n\tthen you have %s GH/s\n\tGiving you %s of all BTC mining power" % (
    petaall,
    currentinths * petaall,
    hashper * petaall,
)
